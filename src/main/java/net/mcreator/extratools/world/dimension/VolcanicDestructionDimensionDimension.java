...
package net.mcreator.extratools.world.dimension;
...
import net.mcreator.extratools.item.VolcanicDestructionDimensionItem;
...
public class VolcanicDestructionDimensionDimension extends ExtraToolsModElements.ModElement {
	@ObjectHolder("extra_tools:volcanic_destruction_dimension_portal")
	public static final CustomPortalBlock portal = null;
	public VolcanicDestructionDimensionDimension(ExtraToolsModElements instance) {
		super(instance, 122);
		...
		DeferredWorkQueue.runLater(() -> {
			try {
				ObfuscationReflectionHelper.setPrivateValue(WorldCarver.class, WorldCarver.CAVE, new ImmutableSet.Builder<Block>()
						...
						.add(Blocks.CRACKED_POLISHED_BLACKSTONE_BRICKS).build(), "field_222718_j");
	...
	public void clientLoad(FMLClientSetupEvent event) {
		...
		DeferredWorkQueue.runLater(() -> {
			try {
				...
				effectsRegistry.put(new ResourceLocation("extra_tools:volcanic_destruction_dimension"), customEffect);
			} catch (Exception e) {
				e.printStackTrace();
			}
		});
	...
	public static final TicketType<BlockPos> CUSTOM_PORTAL = TicketType.create("volcanic_destruction_dimension_portal", Vector3i::compareTo, 300);
	public static class POIRegisterHandler {
		...
		public void registerPointOfInterest(RegistryEvent.Register<PointOfInterestType> event) {
			poi = new PointOfInterestType("volcanic_destruction_dimension_portal",
					Sets.newHashSet(ImmutableSet.copyOf(portal.getStateContainer().getValidStates())), 0, 1)
							.setRegistryName("volcanic_destruction_dimension_portal");
	...
	public void initElements() {
		...
		elements.items.add(() -> new VolcanicDestructionDimensionItem().setRegistryName("volcanic_destruction_dimension"));
	}
			...
			if (!entity.isPassenger() && !entity.isBeingRidden() && entity.isNonBoss() && !entity.world.isRemote && true) {
				if (entity.func_242280_ah()) {
					entity.func_242279_ag();
						...
						new ResourceLocation("extra_tools:volcanic_destruction_dimension"))) {
					entity.func_242279_ag();
					teleportToDimension(entity, pos,
							RegistryKey.getOrCreateKey(Registry.WORLD_KEY, new ResourceLocation("extra_tools:volcanic_destruction_dimension")));
				} else {
					entity.func_242279_ag();
	...
	public static class CustomPortalSize {
		private static final AbstractBlock.IPositionPredicate POSITION_PREDICATE = (state, blockReader, pos) -> {
			return state.getBlock() == Blocks.CRACKED_POLISHED_BLACKSTONE_BRICKS;
		};
			...
			for (int k1 = -1; k1 < 3; ++k1) {
				for (int i2 = -1; i2 < 4; ++i2) {
					if (k1 == -1 || k1 == 2 || i2 == -1 || i2 == 3) {
						...
						this.world.setBlockState(blockpos$mutable, Blocks.CRACKED_POLISHED_BLACKSTONE_BRICKS.getDefaultState(), 3);
					}
				}
			}
